...
def Analyze_Experiment(experiment):
    ...
    if average:
        ...
        experiment['varied_quantities']
        known_combos = []
        
        for run in experiment['runs']:
            #Get the variables actual values
            varied_set = [run[x] for x in experiment['varied_quantities']]
            print(varied_set)
            
            #
            already_known = False
            for entry in known_combos:
                if varied_set == entry:
                    already_known = True
            
            if not already_known:
                #Add to list and sum others of that type.
                known_combos.append(varied_set)
                print("Known combos")
                ...
                for other_run in experiment['runs']:
                    ...
                    if varied_set == [other_run[x] for x in experiment['varied_quantities']]:
                        print("Found a match")
                        if sum_rpe.size > 0:
    ...
    plt.title("Analytic Policy use Vs. Average Reward")
    plt.ylabel("Reward")
